...
#include<string.h>
int main(){
	char a[110][110][110];		//存储用户输入 行 每行字符串 每个字符串长度 
	char x;					//用户输入 
	int len[110];			//每行字符串个数 
	int i=0;				//行 
	int j=0;				//每行字符串个数 
	...
	for(int k=0;k<i;k++){
		for(int q=0;q<=len[k];q++){
				...
				//strcmp函数只能判断两个字符串是否完全相同，当输入关键字连在一起就无法正常判断 如果输入 chi1 huo3 guo1? 就错误
				//if(strstr(a[k][q],"chi1") != NULL&&strstr(a[k][q+1],"huo3") != NULL&&strstr(a[k][q+2],"guo1") != NULL){
				//strstr函数可以判断两个字符串是否包含 如果输入 chi11 huo3 guo1? 就错误
                if(strcmp(a[k][q],"chi1")==0&&strcmp(a[k][q+1],"huo3")==0&&strstr(a[k][q+2],"guo1") != NULL){	
				//前两个用strcmp严格判断，最后一个用strstr包含（末尾标点符号）
					if(sum==0){
